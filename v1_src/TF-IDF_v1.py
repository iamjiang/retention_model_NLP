...
def main(args,train_data, test_data, device):
    ...
    word_count_vector=cv.fit_transform(df_train[args.feature_name])
    ...
    train_tfidf = vectorizer.fit_transform(df_train[args.feature_name])
    train_tfidf = train_tfidf.toarray()
    vocab = vectorizer.get_feature_names()
    vocab = np.array(vocab)
    train_tfidf = pd.DataFrame(train_tfidf,columns=vocab)
    
    test_tfidf = vectorizer.transform(df_test[args.feature_name])
    test_tfidf = pd.DataFrame(test_tfidf,columns=vocab)

    train_label=df_train["churn"].values.squeeze()
    test_label=df_test["churn"].values.squeeze()
