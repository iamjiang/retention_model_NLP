...
def askunum_textbody(args): 
    ...
    try:
        askunum_text['TextBody'] = askunum_text['TextBody'].progress_apply(lambda x: x.encode("latin1").decode("cp1252")) ### convert unicode strings to string \x92 --> '
    except:
        pass
    ...
    askunum_text['TextBody'] = askunum_text['TextBody'].progress_apply(remove_short_sentence)
    ...
    askunum_text['TextBody'] = askunum_text['TextBody'].str.replace("re[\s?\,?\d]+","", regex=True) # remove strings like re 421678 or re421678
    askunum_text['TextBody'] = askunum_text['TextBody'].str.replace("r[\s?\,?\d]+","", regex=True) # remove strings like r0790774 215989
    ...
    askunum_text.dropna(subset=['TextBody'],inplace=True)
    askunum_text=askunum_text[askunum_text.TextBody.notna()]
    askunum_text=askunum_text[askunum_text.TextBody.str.len()>0]
    askunum_text.sort_values(["unum_id","MessageDate"],inplace=True,ascending=True)
