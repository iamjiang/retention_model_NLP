...
start=time.time()
askunum_text=askunum_text.groupby(["unum_id","year","month","Subtype"])['TextBody'].apply(lambda x: " ".join(x)).reset_index()
askunum_text=askunum_text.drop_duplicates()
end=time.time()
print("It take {:.4f} second to group data".format(end-start))

askunum_text.to_pickle(os.path.join(my_folder,"askunum_text"))
