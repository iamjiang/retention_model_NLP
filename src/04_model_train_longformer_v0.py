...
#     test_data=Dataset.from_pandas(df_test)
    ...
    LABEL=df_train['churn'].values.squeeze()
...
#         accelerator.print(f"\n** avg_loss : {epoch_loss:.2f}, time :~ {(time.time()-t0)//60} min ({time.time()-t0 :.2f} sec)***\n")
        ...
        accelerator.print("")
        accelerator.print("avg_loss: {:.2f} | True_Prediction: {:,} | False_Prediction: {:,} | accuracy: {:.2%} | precision: {:.2%} | recall: {:.2%} | F1_score: {:.2%} | Gain_top-1%: {:.1f} | Gain_top-5%: {:.1f} | Gain_top-10%: {:.1f} | ROC_AUC: {:.1%} | PR_AUC: {:.1%} | Elapsed: {:}".\
               format(avg_train_loss, train_output["true_prediction"], train_output["false_prediction"], train_output["accuracy"], \
                     train_output["precision"], train_output["recall"], train_output["f1_score"], train_output["GAIN"]['1%'], train_output["GAIN"]['5%'], \
                     train_output["GAIN"]['10%'], train_output["AUC"], train_output["pr_auc"], \
                      utils.format_time(t2-t1)))
        if accelerator.is_main_process:
            gain_1=train_output["GAIN"]["1%"]
            gain_5=train_output["GAIN"]["5%"]
            gain_10=train_output["GAIN"]["10%"]
            with open(os.path.join(os.getcwd(),"training_output.txt"),'a') as f:
        ...
        t3=time.time()
        val_pred,val_target,val_losses=utils.eval_func(valid_dataloader,model,accelerator.device)

        avg_val_loss=np.mean(val_losses)

        val_output=utils.model_evaluate(val_target.reshape(-1),val_pred)

        t4=time.time()
        accelerator.print("")
        accelerator.print("==> Running Validation on validation set \n")
        accelerator.print("")
        accelerator.print("avg_loss: {:.2f} | True_Prediction: {:,} | False_Prediction: {:,} | accuracy: {:.2%} | precision: {:.2%} | recall: {:.2%} | F1_score: {:.2%} | Gain_top-1%: {:.1f} | Gain_top-5%: {:.1f} | Gain_top-10%: {:.1f} | ROC_AUC: {:.1%} | PR_AUC: {:.1%} | Elapsed: {:}".\
               format(avg_val_loss, val_output["true_prediction"], val_output["false_prediction"], val_output["accuracy"], \
                     val_output["precision"], val_output["recall"], val_output["f1_score"], val_output["GAIN"]['1%'], val_output["GAIN"]['5%'], \
                     val_output["GAIN"]['10%'], val_output["AUC"], val_output["pr_auc"], \
                      utils.format_time(t4-t3)))  

        if accelerator.is_main_process:
            gain_1=val_output["GAIN"]["1%"]
            gain_5=val_output["GAIN"]["5%"]
            gain_10=val_output["GAIN"]["10%"]
            with open(os.path.join(os.getcwd(),"validation_output.txt"),'a') as f:
                f.write(f'{args.model_output_name},{epoch},{avg_val_loss},{val_output["true_prediction"]},{val_output["false_prediction"]},{val_output["accuracy"]},{val_output["precision"]},{val_output["recall"]},{val_output["f1_score"]},{gain_1},{gain_5},{gain_10},{val_output["AUC"]},{val_output["pr_auc"]}\n')    
        ...
        accelerator.print("avg_loss: {:.2f} | True_Prediction: {:,} | False_Prediction: {:,} | accuracy: {:.2%} | precision: {:.2%} | recall: {:.2%} | F1_score: {:.2%} | Gain_top-1%: {:.1f} | Gain_top-5%: {:.1f} | Gain_top-10%: {:.1f} | ROC_AUC: {:.1%} | PR_AUC: {:.1%} | Elapsed: {:}".\
               format(avg_test_loss, test_output["true_prediction"], test_output["false_prediction"], test_output["accuracy"], \
                     test_output["precision"], test_output["recall"], test_output["f1_score"], test_output["GAIN"]['1%'], test_output["GAIN"]['5%'], \
                     test_output["GAIN"]['10%'], test_output["AUC"], test_output["pr_auc"], \
                      utils.format_time(t6-t5)))  

        if accelerator.is_main_process:
            gain_1=test_output["GAIN"]["1%"]
            gain_5=test_output["GAIN"]["5%"]
            gain_10=test_output["GAIN"]["10%"]
            with open(os.path.join(os.getcwd(),"test_output.txt"),'a') as f:
                f.write(f'{args.model_output_name},{0},{avg_test_loss},{test_output["true_prediction"]},{test_output["false_prediction"]},{test_output["accuracy"]},{test_output["precision"]},{test_output["recall"]},\
...
#     parser.add_argument('--lr_scheduler_type', type=str, default="cosine")
    ...
    email_all=concatenate_datasets([hf_split["train"],  hf_split["validation"]],split="train")
    email_all=DatasetDict({"train":email_all, "test":hf_split.pop('test')})
