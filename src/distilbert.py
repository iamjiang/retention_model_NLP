...
class CustomBERTModel(nn.Module):
    def __init__(self,args):
        ...
        self.model = AutoModel.from_pretrained(args.model_checkpoint)
...
#     )
    ...
    for epoch in tqdm(range(args.num_epochs),position=0 ,leave=True):
        ...
        for step,batch in enumerate(train_dataloader):
            ...
            logits=model(batch,accelerator.device)
#             loss=outputs.loss
...
#             print(step,outputs,logits.view(-1, num_classes))
            ...
            if loss_weight is None:
                ...
                loss = F.cross_entropy(logits.view(-1, num_classes).to(accelerator.device), batch["labels"], weight=loss_weight.float().to(accelerator.device)) 
...
#         accelerator.print(f"\n** avg_loss : {epoch_loss:.2f}, time :~ {(time.time()-t0)//60} min ({time.time()-t0 :.2f} sec)***\n")
        ...
        t1=time.time()
        train_pred,train_target,train_losses=eval_func(train_dataloader,model, accelerator.device,num_classes=num_classes, loss_weight=loss_weight)
        ...
        test_pred,test_target,test_losses=eval_func(test_dataloader,model,accelerator.device,num_classes=num_classes,loss_weight=loss_weight)
...
if __name__=="__main__":
    ...
    parser.add_argument("--output_dir", default=os.path.join(os.getcwd(),"distilbert_repo"), type=str, help="output folder name")
    parser.add_argument("--model_output_name", default="distilbert", type=str)
...
#     email_all=email_all.rename_column("truncated_text", args.feature_name)
    ...
    # train_data=email_all['train'].shuffle(seed=101).select(range(5000))
