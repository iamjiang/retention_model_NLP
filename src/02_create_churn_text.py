...
def main(args,churn_df,text_df):
    ...
    unique_unum_id=pd.read_csv(os.path.join(my_folder,file_name+".csv"))['0'].values
    ...
    for index,row in tqdm(churn_df.iterrows(), total=churn_df.shape[0]):
        ...
        tempt=text_df[(text_df["unum_id"]==row["unum_id"]) & (text_df["MessageDate"].dt.date>=row["start_date"]) &  (text_df["MessageDate"].dt.date<=row["end_date"])]
