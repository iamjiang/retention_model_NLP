...
def main(args,train_data, test_data, device):
    ...
    model=AutoModelForSequenceClassification.from_pretrained(args.model_checkpoint, num_labels = num_classes)
...
#     )
    ...
    #             loss=outputs.loss
        ...
        train_output,_,_=utils.model_evaluate(train_target.reshape(-1),train_pred)
        ...
        accelerator.print("")
        accelerator.print("avg_loss: {:.6f} | accuracy: {:.2%} | F1-Score(micro) {:.2%} | F1-Score(macro) {:.2%} | F1-Score(weighted) {:.2%}  | roc-auc(micro) {:.2%} | roc-auc(macro) {:.2%}  | roc-auc(weighted) {:.2%}  | pr-auc(micro) {:.2%} | pr-auc(macro) {:.2%} | pr-auc(weighted) {:.2%}  | Elapsed: {:}".\
               format(avg_train_loss, train_output["acc"], train_output["fscore_micro"], train_output["fscore_macro"], train_output["fscore_weighted"], \
                     train_output["auc_micro"], train_output["auc_macro_ovo"], train_output["auc_weighted_ovo"], train_output["pr_auc_micro"], train_output["pr_auc_macro"], 
                      train_output["pr_auc_weighted"], \
                      utils.format_time(t2-t1)))
        if accelerator.is_main_process:
            with open(os.path.join(os.getcwd(),"metrics_training.txt"),'a') as f:
                f.write(f'{args.model_output_name},{epoch},{avg_train_loss},{train_output["acc"]},{train_output["fscore_micro"]},{train_output["fscore_macro"]},{train_output["fscore_weighted"]},{train_output["auc_micro"]},{train_output["auc_macro_ovo"]},{train_output["auc_weighted_ovo"]},{train_output["pr_auc_micro"]},{train_output["pr_auc_macro"]},{train_output["pr_auc_weighted"]}\n')    

        t3=time.time()

        test_pred,test_target,test_losses=utils.eval_func(test_dataloader,model,accelerator.device,num_classes=num_classes, loss_weight=loss_weight)
        avg_test_loss=np.mean(test_losses)
        test_output,_,_=utils.model_evaluate(test_target.reshape(-1),test_pred)
        ...
        accelerator.print("")
        accelerator.print("avg_loss: {:.6f} | accuracy: {:.2%} | F1-Score(micro) {:.2%} | F1-Score(macro) {:.2%} | F1-Score(weighted) {:.2%}  | roc-auc(micro) {:.2%} | roc-auc(macro) {:.2%}  | roc-auc(weighted) {:.2%}  | pr-auc(micro) {:.2%} | pr-auc(macro) {:.2%} | pr-auc(weighted) {:.2%}  | Elapsed: {:}".\
               format(avg_test_loss, test_output["acc"], test_output["fscore_micro"], test_output["fscore_macro"], test_output["fscore_weighted"], \
                     test_output["auc_micro"], test_output["auc_macro_ovo"], test_output["auc_weighted_ovo"], test_output["pr_auc_micro"], test_output["pr_auc_macro"], 
                      test_output["pr_auc_weighted"], \
                      utils.format_time(t4-t3)))  

        if accelerator.is_main_process:

            with open(os.path.join(os.getcwd(),"metrics_test.txt"),'a') as f:
                f.write(f'{args.model_output_name},{epoch},{avg_test_loss},{test_output["acc"]},{test_output["fscore_micro"]},{test_output["fscore_macro"]},{test_output["fscore_weighted"]},{test_output["auc_micro"]},{test_output["auc_macro_ovo"]},{test_output["auc_weighted_ovo"]},{test_output["pr_auc_micro"]},{test_output["pr_auc_macro"]},{test_output["pr_auc_weighted"]}\n')    
        if accelerator.is_main_process:
...
if __name__=="__main__":
    ...
    train_df=pd.read_csv(os.path.join(input_dir,"train_df_v1.csv"))
    val_df=pd.read_csv(os.path.join(input_dir,"val_df_v1.csv"))
    test_df=pd.read_csv(os.path.join(input_dir,"test_df_v1.csv"))
    ...
    train_df=cate_2_int_label(train_df,col="Subtype")
    val_df=cate_2_int_label(val_df,col="Subtype")
    test_df=cate_2_int_label(test_df,col="Subtype")
